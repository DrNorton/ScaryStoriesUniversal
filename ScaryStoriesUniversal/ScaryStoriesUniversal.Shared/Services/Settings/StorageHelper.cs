...
namespace ScaryStoriesUniversal.Services.Settings
{
    public static class StorageHelper
    {
        ...
        public static T GetCompositeSetting<T>(string key, T otherwise = default(T), StorageStrategies location = StorageStrategies.Local)
        {
            try
            {
                if (!(SettingExists(key, location)))
                ...
                var tesl =
                         Windows.Storage.ApplicationData.Current.LocalSettings.Values[key.ToString()].ToString();
                switch (location)
                {
                     
                    case StorageStrategies.Local:
                        return Deserialize<T>(Windows.Storage.ApplicationData.Current.LocalSettings.Values[key.ToString()].ToString());
                    case StorageStrategies.Roaming:
                        return Deserialize<T>(Windows.Storage.ApplicationData.Current.RoamingSettings.Values[key.ToString()].ToString());
                    default:
                        throw new NotSupportedException(location.ToString());
                }
            }
        ...
        public static void SetCompositeSetting<T>(string key, T value, StorageStrategies location = StorageStrategies.Local)
        {
            switch (location)
            {
                case StorageStrategies.Local:
                    Windows.Storage.ApplicationData.Current.LocalSettings.Values[key.ToString()] = Serialize(value);
                    break;
                case StorageStrategies.Roaming:
                    Windows.Storage.ApplicationData.Current.RoamingSettings.Values[key.ToString()] = Serialize(value);
                    break;
                default:
                    throw new NotSupportedException(location.ToString());
            }
